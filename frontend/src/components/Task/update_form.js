...
const UpdateForm = ({ data, project_id, issue_id }) => {
  ...
  const Descrip = ContentState.createFromBlockArray(
    contentHTML.contentBlocks,
    contentHTML.entityMap
  );

  const contentDes = JSON.stringify(convertToRaw(Descrip));

  const [content, setContent] = useState(contentDes);

  const [chipval, setChipval] = useState([data.group]);
